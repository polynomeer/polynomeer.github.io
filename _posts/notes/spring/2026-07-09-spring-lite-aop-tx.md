...
트랜잭션을 공부할 때는 자꾸 DB 쪽으로 먼저 시선이 간다. 커밋, 롤백, 전파, 격리 수준 같은 단어가 더 눈에 띄기 때문이다. 그런데 `spring-lite`를 만들고 따라가면서 제일 먼저 바뀐 건 시선의 순서였다. DB보다 먼저 프록시를 봐야 했다.
...
처음엔 이 부분이 자잘한 유틸리티처럼 보였는데, 구현을 따라가다 보니 꽤 중요했다. `JdbcTemplate` 쪽에서는 평범하게 `try-with-resources`를 쓰더라도, 트랜잭션이 소유한 연결은 중간에 닫히면 안 된다.
